...
del_table_2() ->
    ?assertEqual({error, no_such_table_entry}, etsmgr:del_table(table1)),
    true.
...
del_table_3() ->
    ...
    ?assertEqual(self(), ets:info(Table_id1, owner)),
    ?assertEqual(Mgr_pid, ets:info(Table_id1, heir)),
    ...
    ?assertEqual(self(), ets:info(Table_id2, owner)),
    ?assertEqual(Mgr_pid, ets:info(Table_id2, heir)),
    ...
    ?assertEqual({ok, Mgr_pid}, etsmgr:del_table(table2)),
    ?assert(ets:delete(Table_id2)),
    true.
...
del_table_test_() ->
    ...
    {"delete table",
     [{setup, fun setup/0, fun cleanup/1,
       {inorder,
        [ {"simple create/delete", ?_assert(del_table_1())},
          {"deleting a non-existant table", ?_assert(del_table_2())},
          {"deleting one of multiple table", ?_assert(del_table_3())}
        ]}
      }]
    }.
